...
public class XListViewFooter extends LinearLayout {
	...
	public void setState(int state) {
		mHintView.setVisibility(View.INVISIBLE);// �?始底部控件都隐藏
		mProgressBar.setVisibility(View.INVISIBLE);
		mHintView.setVisibility(View.INVISIBLE);
		if (state == STATE_READY) {// 如果是第�?页状态，那么“查看更多�?�显�?
			mHintView.setVisibility(View.VISIBLE);
			mHintView.setText(R.string.xlistview_footer_hint_ready);// 松开显示更多
		} else if (state == STATE_LOADING) {// 当加载的时�??
			mProgressBar.setVisibility(View.VISIBLE);// 加载进度条显�?
		} else {
			mHintView.setVisibility(View.VISIBLE);
