...
public class DevMountInfo {
	 ...
	 *            数据初始化是否同步执�?
	 */
	public void init(final Context context, boolean isSync) {
	...
	public String getSDCardPath() {
		return mSDCardPath = mSDCardPath.replace("/", File.separator);
	}
	...
	private void executeInit(Context context) {
		...
		if (mStorageManager != null) {
			...
			try {
				...
				mGetVolumeListMethod = mStorageManager.getClass().getMethod(
						METHOD_GET_VOLUME_LIST, new Class[0]);
				mGetVolumeStateMethod = mStorageManager.getClass().getMethod(
						METHOD_GET_VOLUME_STATE, new Class[] { String.class });
				mIsRemovableMethod = mStorageVolume.getMethod(
						METHOD_IS_REMOVABLE, new Class[0]);
				mGetPathMethod = mStorageVolume.getMethod(METHOD_GET_PATH,
						new Class[0]);

				mStorageVolumeList = (Object[]) mGetVolumeListMethod.invoke(
						mStorageManager, new Object[0]);
				...
				if (mStorageVolumeList != null && mStorageVolumeList.length > 0) {
					...
					// if(Env.DEG){
					// Log.i(TAG, "init() === > StorageVolume Count = " +
					// mStorageVolumeCount);
					// }
					...
					for (int i = 0; i < mStorageVolumeCount; ++i) {
						String mStoragePath = (String) mGetPathMethod.invoke(
								mStorageVolumeList[i], new Object[0]);
						mIsRemovable = ((Boolean) mIsRemovableMethod.invoke(
								mStorageVolumeList[i], new Object[0]))
								.booleanValue();
						if (!TextUtils.isEmpty(mStoragePath)) {
							String state = (String) mGetVolumeStateMethod
				...
				// if(Env.DEG){
				// Log.e(TAG,
				// "init() === > Exception:IllegalArgumentException");
				// }
				...
				// if(Env.DEG){
				// Log.e(TAG,
				// "init() === > Exception:InvocationTargetException");
				// }
			}
		} else {
			handleInvalid();
	...
	private void handleInvalid() {
		mInternalPathList.add(Environment.getExternalStorageDirectory()
				.getPath());
	}
	...
	private void initSDCardPath() {
		if (!mExternalPathList.isEmpty()) {
			...
			mSDCardPath = mInternalPathList.peek().replace("/", File.separator);
		} else {
			mSDCardPath = Environment.getExternalStorageDirectory().getPath()
					.replace("/", File.separator);
			;
		}
		mSDCardPath = mSDCardPath.replace("/", File.separator);
		// if(Env.DEG){
